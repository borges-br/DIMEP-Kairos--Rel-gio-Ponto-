...
async function localData(client: PoolClient, organizationId: string) {
  const [clients, collaborators, projects, tasks] = await Promise.all([
    ...
    client.query(`select er.external_id,c.full_name,c.cpf_digits,c.active from rdo.collaborator_external_refs er
                   join rdo.collaborators c on c.id=er.collaborator_id where er.organization_id=$1`, [organizationId]),
...
export async function applyImuvPull(client: PoolClient, organizationId: string, userId: string, data: ImuvData, preview: ImuvPreview) {
  ...
  const runId = runResult.rows[0].id; let written = 0; let rejected = preview.counts.skip;
  ...
  for(const row of data.projects){const id=asText(row.id);if(!id)continue;const clientId=clientIds.get(asText(row.people_id)||"");if(!clientId){rejected+=1;continue;}const name=asText(row.name)||`Projeto ${id}`;const code=asText(row.code)||id;const completed=Boolean(date(row.date_finished));
