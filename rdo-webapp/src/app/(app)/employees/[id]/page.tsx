...
export default async function EmployeeDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ correction?: string }> }) {
  ...
  return <div className="page-container wide-page">
    ...
    <section className="detail-summary"><article><ProjectsIcon /><span><small>Projetos vinculados</small><strong>{detail.projects.length}</strong></span></article><article><ClockIcon /><span><small>Apontamentos</small><strong>{detail.workHistory.length}</strong></span></article><article><WarningIcon /><span><small>Divergências pendentes</small><strong>{detail.workHistory.filter((item) => item.divergence_status === "pending").length + detail.dimepIssues.length}</strong></span></article></section>
