...
export default async function HoursPage() {
  ...
  return <div className="page-container">
    <Breadcrumbs items={[{ label: "Início", href: "/" }, { label: "Apontamentos de horas" }]} />
    ...
    {rows.length ? <div className="table-shell"><table className="data-table"><thead><tr><th>Data</th><th>Colaborador</th><th>Origem / etapa</th><th>Tempo</th><th>Linhas</th><th>Conciliação</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.source}-${row.work_date}-${row.collaborator_name}`}><td>{formatDate(row.work_date)}</td><td><strong>{row.collaborator_name}</strong></td><td>{row.source === "dimep" ? <span className="status-badge status-neutral">DIMEP · aguardando RDO</span> : <span className="status-badge status-success">RDO · {row.status}</span>}</td><td>{formatMinutes(row.total_minutes)}</td><td>{row.allocation_count}</td><td>{Number(row.divergence_count) ? <span className="status-badge status-warning"><WarningIcon />{row.divergence_count} divergência(s)</span> : <span className="status-badge status-success">Coberto</span>}</td></tr>)}</tbody></table></div> : <div className="empty-state panel"><ClockIcon /><h2>Sem jornadas importadas</h2><p>Execute a sincronização DIMEP. Depois, as jornadas aguardando distribuição e os RDOs aparecerão aqui.</p></div>}
  </div>;
}
