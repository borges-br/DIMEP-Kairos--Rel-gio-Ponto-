...
export default async function HoursPage() {
  ...
  return <div className="page-container">
    ...
    {rows.length ? <div className="table-shell"><table className="data-table"><thead><tr><th>Data</th><th>Colaborador</th><th>Tempo alocado</th><th>Linhas</th><th>Conciliação</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.work_date}-${row.collaborator_name}`}><td>{formatDate(row.work_date)}</td><td><strong>{row.collaborator_name}</strong></td><td>{formatMinutes(row.total_minutes)}</td><td>{row.allocation_count}</td><td>{Number(row.divergence_count) ? <span className="status-badge status-warning"><WarningIcon />{row.divergence_count} divergência(s)</span> : <span className="status-badge status-success">Coberto</span>}</td></tr>)}</tbody></table></div> : <div className="empty-state panel"><ClockIcon /><h2>Sem apontamentos</h2><p>As horas aparecem depois que um RDO é registrado.</p></div>}
  </div>;
}
