...
export default async function RdoDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ workflow?: string }> }) {
  ...
  return <div className="page-container wide-page">
    <Breadcrumbs items={[{ label: "Início", href: "/" }, { label: "Diários de campo", href: "/rdos" }, { label: `${detail.rdo.project_code} · ${formatDate(detail.rdo.work_date)}` }]} />
    <header className="page-header"><div><span className="eyebrow">{detail.rdo.project_code} · VERSÃO {detail.rdo.version_number}</span><h1>{detail.rdo.project_name}</h1><p>{detail.rdo.client_name} · {formatDate(detail.rdo.work_date)} · Líder: {detail.rdo.leader_name}</p></div><StatusBadge status={detail.rdo.status} /></header>
    ...
    <section className="panel detail-box detail-wide"><h2>Evidências e áudio</h2><p>Fotos e áudios ficam no MinIO; o PostgreSQL conserva metadados, vínculos, hash e auditoria.</p><MediaManager rdoId={detail.rdo.id} canUpload={canUpload} activities={detail.activities} media={detail.media} /></section>
